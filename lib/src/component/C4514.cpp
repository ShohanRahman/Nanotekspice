...
// Last update Sun Feb 21 15:49:31 2016 avelin_j
//
...
void nts::C4514::SetLink(size_t pin_num_this,
...
{
  std::cout << "Chipset 4514" << std::endl;
  for (int i = 0 ; i < 14 ; i++)
    {
      std::cout << "\tpin #" << i + 1 << ": ";
      if (_pin[i])
...
nts::Tristate nts::C4514::compute_nand(nts::Tristate lhs, nts::Tristate rhs) const
{
...
nts::Tristate nts::C4514::computeInput(size_t pin) const
{
  if (_pin[pin - 1])
    {
...
nts::Tristate nts::C4514::computeOutput(size_t pin) const
{
...
nts::Tristate nts::C4514::computeVDD(size_t pin) const
{
  (void)pin;
...
nts::Tristate nts::C4514::computeVSS(size_t pin) const
{
  (void)pin;
